#include "commands.h"
#include "data_buffer.h"        // conduit_dtype_t
...
typedef struct {
    const char *name;
    conduit_cmd_handler_t handler;     // NULL when typed_cb is set
    void              *typed_cb;   // cast back via typed_dtype
    conduit_dtype_t        typed_dtype;
} cmd_entry_t;

static cmd_entry_t g_cmds[CONDUIT_CMD_MAX];
...
static cmd_entry_t *cmd_slot(const char *name) {
    ...
    if (g_cmd_count >= CONDUIT_CMD_MAX) return NULL;
...
static void register_typed(const char *name, void *cb, conduit_dtype_t dtype) {
    if (!name || !cb) return;
    cmd_entry_t *e = cmd_slot(name);
    if (!e) return;
...
void _poe_register_cb_i8 (const char *name, conduit_cmd_cb_i8_t  cb) { register_typed(name, (void *)cb, CONDUIT_DTYPE_I8);  }
void _poe_register_cb_u8 (const char *name, conduit_cmd_cb_u8_t  cb) { register_typed(name, (void *)cb, CONDUIT_DTYPE_U8);  }
void _poe_register_cb_i16(const char *name, conduit_cmd_cb_i16_t cb) { register_typed(name, (void *)cb, CONDUIT_DTYPE_I16); }
void _poe_register_cb_u16(const char *name, conduit_cmd_cb_u16_t cb) { register_typed(name, (void *)cb, CONDUIT_DTYPE_U16); }
void _poe_register_cb_i32(const char *name, conduit_cmd_cb_i32_t cb) { register_typed(name, (void *)cb, CONDUIT_DTYPE_I32); }
void _poe_register_cb_u32(const char *name, conduit_cmd_cb_u32_t cb) { register_typed(name, (void *)cb, CONDUIT_DTYPE_U32); }
void _poe_register_cb_i64(const char *name, conduit_cmd_cb_i64_t cb) { register_typed(name, (void *)cb, CONDUIT_DTYPE_I64); }
void _poe_register_cb_u64(const char *name, conduit_cmd_cb_u64_t cb) { register_typed(name, (void *)cb, CONDUIT_DTYPE_U64); }
void _poe_register_cb_f32(const char *name, conduit_cmd_cb_f32_t cb) { register_typed(name, (void *)cb, CONDUIT_DTYPE_F32); }
void _poe_register_cb_f64(const char *name, conduit_cmd_cb_f64_t cb) { register_typed(name, (void *)cb, CONDUIT_DTYPE_F64); }
...
static int invoke_typed(const cmd_entry_t *e, const char *args,
    ...
    switch (e->typed_dtype) {
        case CONDUIT_DTYPE_I8: {
            int8_t v = (int8_t)conduit_cmd_arg_int(args, "value", 0);
            err = ((conduit_cmd_cb_i8_t)e->typed_cb)(v);
            if (!err) written = snprintf(out, out_max, "{\"ok\":true,\"value\":%d}", (int)v);
            break;
        }
        case CONDUIT_DTYPE_U8: {
            uint8_t v = (uint8_t)conduit_cmd_arg_int(args, "value", 0);
            err = ((conduit_cmd_cb_u8_t)e->typed_cb)(v);
            if (!err) written = snprintf(out, out_max, "{\"ok\":true,\"value\":%u}", (unsigned)v);
            break;
        }
        case CONDUIT_DTYPE_I16: {
            int16_t v = (int16_t)conduit_cmd_arg_int(args, "value", 0);
            err = ((conduit_cmd_cb_i16_t)e->typed_cb)(v);
            if (!err) written = snprintf(out, out_max, "{\"ok\":true,\"value\":%d}", (int)v);
            break;
        }
        case CONDUIT_DTYPE_U16: {
            uint16_t v = (uint16_t)conduit_cmd_arg_int(args, "value", 0);
            err = ((conduit_cmd_cb_u16_t)e->typed_cb)(v);
            if (!err) written = snprintf(out, out_max, "{\"ok\":true,\"value\":%u}", (unsigned)v);
            break;
        }
        case CONDUIT_DTYPE_I32: {
            int32_t v = (int32_t)conduit_cmd_arg_long(args, "value", 0);
            err = ((conduit_cmd_cb_i32_t)e->typed_cb)(v);
            if (!err) written = snprintf(out, out_max, "{\"ok\":true,\"value\":%" PRId32 "}", v);
            break;
        }
        case CONDUIT_DTYPE_U32: {
            uint32_t v = (uint32_t)conduit_cmd_arg_long(args, "value", 0);
            err = ((conduit_cmd_cb_u32_t)e->typed_cb)(v);
            if (!err) written = snprintf(out, out_max, "{\"ok\":true,\"value\":%" PRIu32 "}", v);
            break;
        }
        case CONDUIT_DTYPE_I64: {
            // conduit_cmd_arg_long returns long, which is 32-bit on Pico's
        ...
        case CONDUIT_DTYPE_F32: {
            float v = conduit_cmd_arg_float(args, "value", 0.0f);
            err = ((conduit_cmd_cb_f32_t)e->typed_cb)(v);
            if (!err) written = snprintf(out, out_max, "{\"ok\":true,\"value\":%g}", (double)v);
            break;
        }
        case CONDUIT_DTYPE_F64: {
            double v = conduit_cmd_arg_double(args, "value", 0.0);
            err = ((conduit_cmd_cb_f64_t)e->typed_cb)(v);
