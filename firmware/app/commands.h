...
// conduit_setup(). Use the typed `on_command(name, T, cb)` macro
// (preferred — see conduit_user.h) or the raw `conduit_command_register`
...
// them fast — flip a flag and do the heavy work in conduit_loop().
...
// fixed (CONDUIT_CMD_MAX); excess registrations are silently dropped.
...
// -----------------------------------------------------------------------
// Typed command callbacks — see conduit_user.h `on_command(name, T, cb)`
...
// store (cb, dtype) into the same table as conduit_command_register and route
